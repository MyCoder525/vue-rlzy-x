    ...
    <el-form label-width="220px">
      ...
      <div class="block">
        ...
        <el-form-item label="入学时间">
          <el-date-picker v-model="formData.enrolmentTime" type="date" placeholder="请输入时间" class="inputW"
            value-format="yyyy-MM-dd" />
        </el-form-item>
        <el-form-item label="毕业时间">
          <el-date-picker v-model="formData.graduationTime" type="date" placeholder="请输入时间" class="inputW"
            value-format="yyyy-MM-dd" />
        </el-form-item>
...
export default {
  ...
  methods: {
    async savePersonal() {
      const fileList = this.$refs.avatUploadref.fileList;
      if (fileList.some((item) => !item.upload)) {
        return this.$message.warning("图片没上传完成");
      }
    ...
    async saveUserDetailById() {
      ...
      this.$refs.avatUploadref.fileList = [
        {
